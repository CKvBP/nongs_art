...
Production uses Gmail SMTP (TLS port 465). Add the variables in `.env.example` to Vercel **Production**: `GMAIL_USER=nong@buedingen.us`, `EMAIL_FROM=hello@nongsartden.com`, and `STUDIO_NOTIFICATION_EMAIL=nong@buedingen.us`. Set `GMAIL_APP_PASSWORD` and a random 32-byte `CRON_SECRET` as secrets. Enable 2-Step Verification and create an app password directly in Nong's Google account. Configure `hello@nongsartden.com` under Gmail Settings → Accounts → Send mail as. Do not use a normal account password. Set `EMAIL_ENABLED=true` and redeploy once credentials are ready. Preview deployments never send mail.

New submissions atomically queue a customer acknowledgment and a studio alert in Neon with the record; existing records are not backfilled with acknowledgments. After-response processing tries immediate delivery. A protected Vercel cron runs every five minutes (requires a compatible paid plan) to retry pending messages and enqueue reminders for each reserved class session within the next 24 hours. Times use America/New_York including daylight saving changes. Reminders are revalidated against the current booking and schedule before sending; cancelled or removed sessions are skipped. Bookings made within 24 hours can receive a confirmation and reminder close together. No wedding reminder is sent automatically: inquiries are requests, not confirmed wedding contracts. Administrative status changes do not send customer mail in this version.

Studio settings shows the latest 30 email jobs and a retry button for failed jobs. SMTP acceptance is recorded, not inbox delivery or read status. Messages use stable Message-IDs and transactionally claimed five-minute leases to avoid ordinary duplicate sends; SMTP cannot guarantee exactly-once delivery if the process stops after Gmail accepts a message but before Neon records success. There are five automatic attempts with exponential backoff. Credentials and SMTP response bodies are never exposed in the log. Reference photos stay in the authenticated admin portal. Disable `EMAIL_ENABLED` and redeploy to pause sending without losing the queue.

Validate using `npm test`, `npm run lint`, `npm run typecheck`, `npm run build`, and `node scripts/verify-production.mjs`. The production smoke script explicitly disables email and uses isolated local data. Never use live customer addresses in automated tests. After deployment, submit an authorized test booking and inquiry and check the customer inbox, studio inbox, sender address, and delivery log.
